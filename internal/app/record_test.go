...
func TestSweepAndRecordWriteFailureIsNonFatal(t *testing.T) {
	...
	wantStats, wantErr := RunOnce(context.Background(), p, nil, nil, allTiers(), pre, 1<<40, logger)
	if wantErr != nil {
		t.Fatalf("baseline RunOnce failed: %v", wantErr)
	}

	stats, err := SweepAndRecord(context.Background(), p, nil, nil, allTiers(), pre, 1<<40, "once", statusPath, logger)
	if err != nil {
